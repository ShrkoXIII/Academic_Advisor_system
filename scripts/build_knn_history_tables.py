"""Build the two TRAIN-only historical tables used by KNN.

This script does not train a model and does not read VALID or TEST.  Dataframe
helpers and validation live in :mod:`src.knn_history_helpers` so this entry
point contains only the build flow.
"""
...
DEFAULT_STATUS = RAW_DIR / "v_add_student_degree_status.parquet"
DEFAULT_GRADES = RAW_DIR / "v_acs_grade.parquet"
DEFAULT_OUTPUT_DIR = (
    PROJECT_ROOT / "data" / "artifacts" / "knn" / "2026-08-23_history_v1"
)
...
def main() -> None:
    ...
    student_status = pd.read_parquet(args.status_path, columns=STATUS_COLUMNS)
