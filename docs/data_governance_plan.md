...
|------|----------|--------------------|--------|
| C1 — single writer of `after_fet_eng.parquet` | Resolved by design (D1) | `merge_diploma.py` still contains the executable historical in-place write (line 262) and no superseded docstring. Repo census: `handle_gpa.ipynb` (live writer), `merge_diploma.py` (retired writer, still executable), `split_diagnostics.ipynb` (reader). `diploma_type_bucketing.ipynb` references it only in stale markdown/error text. No live caller invokes `merge_diploma.py`. | **REDESIGNED, ENFORCEMENT PENDING.** Fully resolved only after `merge_diploma.py`'s write is neutralized and a repo-wide grep confirms exactly one writer. Do **not** mark C1 resolved before that. |
| D2 — distinct split generations | Approved | No code reflects it. Both enrichment notebooks still rewrite the shared paths (next row). | **PLANNED, NOT IMPLEMENTED.** |
| C2–C4 — split in-place overwrite chain | To be removed by D2 | **Two** in-place rewriters: `course_difficulty.ipynb` saves `df_train_enriched.to_parquet(TRAIN_PATH)` (+valid/test) and `diploma_type_bucketing.ipynb` saves `df_train.to_parquet(TRAIN_PATH)` (+valid/test) — same `MODEL_DATA_DIR/df_{train,valid,test}.parquet` in, same paths out. | **STILL ACTIVE.** This is the current reality D2 has not yet replaced. C2–C4 remain open until distinct split generations exist in code. |
...
| `select.ipynb` diploma survival assert | Promised in §10 (cheap fix) | Not present. Diploma columns survive only by absence from drop-lists. | **NOT IMPLEMENTED — 7a item.** |
...
- **7a — Code & ownership fixes.** Neutralize `merge_diploma.py`'s in-place write (superseded docstring; confirm single writer of `after_fet_eng`); fix **both** `course_difficulty.ipynb` and `diploma_type_bucketing.ipynb` save cells to distinct paths (D2) and the bucketing stale failure message; add the `select` diploma-survival assert; the diploma-source normalization is already applied (verify only); retire/gate the live `merged.csv` debug write in `handle_gpa`; fix the live hardcoded path in `mark_finish_status_disagreement_diagnostic.ipynb` (§1.4 item 11); guard assertions; data-root guard; fitted-state persistence per contract 14 (lands with the D2 rebuild); D6 remediation as decided.
...
Validated against CLAUDE.md guardrails, `docs/pipeline_rules.md` locked decisions, and `docs/paths_audit.md`. Phases 1–4 complete. D1–D4 are approved contract inputs; the §1.2 status table and §1.4 verification record are binding — C1 enforcement-pending, D2 not implemented, C2–C4 still active in two notebooks. Phase 5 executed as planning only and **approved by user 2026-07-07** (`docs/governance_contracts.md`); D6 resolved as an approved exception the same day. Phase 6 executed as planning only: naming & numbering proposals in `docs/naming_plan.md`, no file renamed, no code or data touched. D5 remains the open decision; it does not block Phase 6 approval but shapes two naming items (outlier notebook/artifact) and 7c scoping.

**STOP — Phase 6 approval gate: review `docs/naming_plan.md`, decide D5, then approve Phase 6 to unlock Phase 7 planning-to-execution.**
