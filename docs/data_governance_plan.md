...
|------|----------|--------------------|--------|
| C1 — single writer of `after_fet_eng.parquet` | Resolved by design (D1) | **7a applied:** `merge_diploma.py` now carries a SUPERSEDED docstring and a module-level `raise RuntimeError` before any import/I/O — the historical in-place write is unreachable. Repo census post-edit: `handle_gpa.ipynb` is the only executable writer; `split_diagnostics.ipynb` the reader; no importer/caller of `merge_diploma` exists in src/tests/notebooks. | **RESOLVED — CODE-ENFORCED (7a, 2026-07-07).** Verified by import-guard test + repo-wide grep (see 7a report). Final re-confirmation rides Phase 8. |
| D2 — distinct split generations | Approved | **7a applied (code side):** `split_diagnostics` writes `df_{split}_base.parquet`; `course_difficulty` reads `_base`, writes `df_{split}_difficulty.parquet`; `diploma_type_bucketing` reads `_difficulty`, writes `df_{split}_final.parquet`. | **IMPLEMENTED IN CODE (7a).** On-disk generations do NOT exist yet — building them is the gated 7c rebuild. Training-side readers still point at the bare names (deliberate — see below). |
| C2–C4 — split in-place overwrite chain | Removed by D2 | No notebook writes another stage's path anymore; no writer of the bare `df_{train,valid,test}.parquet` names remains in code. | **REMOVED IN CODE (7a, 2026-07-07).** The bare-named on-disk artifacts are unchanged and remain the live training inputs until 7b/7c migrates readers and rebuilds. |
| `02_merge_diploma` notebook guards | Required (§4 contract) | Guards verified present in code (static inspection): input-existence asserts; required-column + zero-null asserts on the diploma source; `student_id` dtype-`string` asserts on **both** sides; source uniqueness assert + `validate='many_to_one'`; `diploma_type_id` suffix-consistency check with STOP; double-merge guard (STOP if diploma columns already present); post-join row-count invariance assert; match diagnostics. | **GUARDS VERIFIED (static, 2026-07-07).** Runtime behavior still unverified — execution is a gated 7c step. |
| `select.ipynb` diploma survival assert | Promised in §10 (cheap fix) | **7a applied:** save cell asserts `diploma_gpa`/`diploma_type_id` present in `df_feature2` before writing. | **IMPLEMENTED (7a, 2026-07-07).** Static only; fires at next execution. |
| Data-root guard (contract 12) | Required | **7a applied:** `src/paths.py` gains `DATA_DIR_FROM_ENV`, `_DATA_DIR_PREEXISTED` (captured before the mkdir loop) and `assert_data_root(*required)`; wired into the three split-stage notebooks. | **IMPLEMENTED (7a) — partial wiring.** Training/inference entry points not yet wired (remaining non-conformance; later 7a sub-group or 7c pre-check). |
| Deliberate transition state — bare-name readers | — | `light_gbm.ipynb`, `results_analysis.ipynb`, `course_difficulty_fallback_diagnostic.ipynb`, `model_eng/read.ipynb` still read the bare `df_{train,valid,test}.parquet` — the current valid on-disk data. | **PENDING 7b/7c.** Repointing them before the generations exist on disk would break training against current data; they migrate with the rebuild. |
...
**Current position:** Phases 1–4 complete. Revision 4 corrected and promoted 2026-07-07. Phase 5 approved (user, 2026-07-07; `docs/governance_contracts.md`). Phase 6 approved (user, 2026-07-07; `docs/naming_plan.md`); D5 and D6 both resolved, D7 (KNN index role) opened. **Phase 7a (code & ownership fixes) authorized and executed 2026-07-07 — 7b and 7c NOT authorized yet.** See §12 and the 7a report.
...
- **7b — Relocations & renames.** Move `after_fet_eng` and the selected-population artifact out of `AUDIT_DIR`; apply Phase 6 names; repoint the two diagnostic readers of the old `features/` path; clean stale hardcoded-path markdown notes. Copy never cut; repoint consumers only after ownership confirmed; no silent overwrite.
- **7c — Selective rebuild.** Distinct approval gate. Run the Chain A pre-check (§4.1) FIRST. Rebuild starts from the earliest actually-affected stage: diploma-source preprocessing → `02_merge_diploma` → downstream. Do NOT rebuild `01_merge_crg_add_acd`'s logic — it reads only cleaned CRG/ADD/ACD, unaffected by the diploma-source fix. Its `MERGE_DIR` output alignment is **already applied and verified (§1.3, §1.4 item 1) — verification only, do not re-schedule.** Validation per §4.1.
...
- C1 multi-writer of `after_fet_eng` → redesigned (D1), **enforcement pending** (`merge_diploma.py`'s executable in-place write still on disk; single-writer grep still required). NOT resolved.
- C2–C4 split overwrite → to be removed by D2; **still active in code** in BOTH `course_difficulty.ipynb` and `diploma_type_bucketing.ipynb` until distinct split generations exist.
...
- **Broken diploma-source rebuild read (NEW flag, 2026-07-07).** `Academic_info_clean.ipynb` reads `RAW_DIR/v_add_academic_info.parquet`, but the extractor writes `RAW_DIR/v_add_adcademic_info.parquet` (typo "adcademic") — and disk verification shows **only the typo'd file exists** in the active root. Rerunning the cleaning notebook as committed fails at the read; Chain A stage 1 cannot execute today. The cleaned artifact on disk predates the read-path change. Blocks 7c until fixed — either repoint the notebook to the typo'd name (7a) or, preferred, rename the raw artifact to the correct spelling and repoint writer+reader together (7b; proposed in `docs/naming_plan.md`). Do not run the extractor or cleaner before this is resolved.
...
- **Bare split-filename generation risk.** Under D2, if base splits keep the bare names `df_{train,valid,test}.parquet`, the existing readers (`light_gbm`, `results_analysis`, `course_difficulty_fallback_diagnostic`, `model_eng/read`) would silently read the wrong generation after the rebuild. The Phase 6 naming plan therefore retires the bare names entirely — stale readers must fail loudly, not read base data as if final.
...
- **D6** — stays closed as the explicitly approved exception recorded above; not reopened.
...
Validated against CLAUDE.md guardrails, `docs/pipeline_rules.md` locked decisions, and `docs/paths_audit.md`. Phases 1–4 complete. D1–D4 are approved contract inputs. Phase 5 approved (user, 2026-07-07); D6 resolved as an approved exception the same day. Phase 6 approved (user, 2026-07-07) with D5 resolved (outlier branch exploratory/inactive) and the dispositions recorded in §10. D7 (KNN index role) is the open decision.

**Gate-interpretation change (recorded explicitly, 2026-07-07).** The pre-correction Revision 4 wording made D5 a blocker for a "clean Phase 5" approval. At Phase 5 execution the gate was interpreted differently — open decisions D5/D6 do not block drafting or approving the contracts; they block only the later 7c rebuild scoping — and Phase 5 was approved under that interpretation. This is a deliberate, recorded change of gate interpretation, not an oversight or contradictory wording. Both decisions were subsequently resolved (D6: approved exception; D5: exploratory/inactive), so the change has no residual effect.

**Phase 7 authorization state:** 7a (code & ownership fixes) authorized by user 2026-07-07 and executed the same day — see the 7a report and updated §1.2 statuses. **7b (relocations & renames) and 7c (selective rebuild) are NOT authorized; no artifact has been moved, renamed, archived, or rebuilt.**
